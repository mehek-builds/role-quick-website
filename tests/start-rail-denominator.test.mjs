...
/* "notifications" is back as its own step (Mehek, 2026-09-07), positioned after "plan": the
   backend will not accept a `plan` acknowledgement without a verified card on file, so this order
   is also the payment gate for the last screen of setup. */
const ALWAYS = ["focus", "resume", "match", "questions", "review", "trial", "plan", "notifications", "done"];
...
test("every conditional step has its own server signal, and no unconditional step is marked one", async () => {
  ...
  assert.deepEqual(
    conditional,
    ["match", "questions", "sponsorship", "review", "trial", "plan", "notifications"],
  );
