 ...
 * @returns {{file: string, term: string, instead: string, line: number, text: string}[]}
 */
export function findRetired(files) {
  const hits = [];
  for (const { path, source } of files) {
    const lines = stripComments(source).split("\n");
    const allowed = new Set(
      source.split("\n").flatMap((l, i) => (/vocab-allow/.test(l) ? [i + 1] : []))
    );
    ...
    for (const [term, instead] of RETIRED) {
      ...
      const idx = lines.findIndex(
        (l, i) => l.toLowerCase().includes(needle) && !allowed.has(i + 1)
      );
      if (idx === -1) continue;
      hits.push({
        file: path,
