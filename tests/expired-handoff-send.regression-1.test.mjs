...
test("the way out the sentence promises is a real control bound to the restart flag", async () => {
  ...
  assert.match(dashboard, /body: JSON\.stringify\(\{ questions: finalQuestions, \.\.\.\(options\.restart \? \{ restart: true \} : \{\}\),\s*\.\.\.\(options\.workdayPageApprovalToken \? \{ workday_page_approval_token: options\.workdayPageApprovalToken \} : \{\}\) \}\)/);
