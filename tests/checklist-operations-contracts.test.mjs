...
test("billing return verifies the account record and handles cancellation and timeout", () => { const page = read("app/billing/return/page.tsx"); assert.match(page, /api<Me>\("\/me"\)/); assert.match(page, /getBillingReceipt/); assert.match(page, /No payment was confirmed/); assert.match(page, /Payment could not be confirmed yet/); assert.match(page, /exact amount Stripe confirmed/); });
