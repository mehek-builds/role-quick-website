...
test("the controlled portal mirrors every supported adapter without an employer destination", async () => {
  ...
  assert.match(page, /const ripplingFieldNames = newRipplingFieldNames\(\)/);
  assert.match(page, /return <PortalForm board=\{board\} caseId=\{caseId\} ripplingFieldNames=\{ripplingFieldNames\} \/>/);
  assert.doesNotMatch(page, /useSearchParams|Suspense/);
  assert.match(casePage, /const ripplingFieldNames = newRipplingFieldNames\(\)/);
  assert.match(casePage, /return <PortalForm board=\{board\} caseId=\{caseId\} ripplingFieldNames=\{ripplingFieldNames\} \/>/);
