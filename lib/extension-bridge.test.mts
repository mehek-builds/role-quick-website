...
test("an extension that answers 'not signed in' is handed this session", async () => {
  reset((message) => {
    ...
    if (typed.type === "LITOS_PING") return { ok: true, signedIn: false, version: "0.6.5" };
  ...
  assert.deepEqual(state, { installed: true, signedIn: true, otherAccount: false, version: "0.6.5", updateRequired: false });
...
test("an extension that already has a session still verifies the current website account", async () => {
  reset((message) => (message as { type: string }).type === "LITOS_PING"
    ? { ok: true, signedIn: true, version: "0.6.5" }
...
test("a guest session is never handed over", async () => {
  reset(() => ({ ok: true, signedIn: false, version: "0.6.5" }));
  const state = await ensureExtensionSession({ token: "guest-jwt", guest: true });
  assert.deepEqual(state, { installed: true, signedIn: false, otherAccount: false, version: "0.6.5", updateRequired: false });
  assert.equal(
...
test("an extension signed in to someone else is reported rather than overridden", async () => {
  reset((message) => {
    const typed = message as { type: string };
    if (typed.type === "LITOS_PING") return { ok: true, signedIn: true, version: "0.6.5" };
    return { ok: false, outcome: "different_account" };
  });
  const state = await ensureExtensionSession({ token: "jwt-abc", guest: false });
  assert.deepEqual(state, { installed: true, signedIn: false, otherAccount: true, version: "0.6.5", updateRequired: false });
});
...
test("the attended path performs a fresh identity check", async () => {
  ...
  reset((message) => {
    ...
    if (type === "LITOS_PING") return { ok: true, signedIn: true, version: "0.6.5" };
...
test("the published 0.6.4 client is installed but held for an update", async () => {
  reset(() => ({ ok: true, signedIn: true, version: "0.6.4" }));
...
test("every attended employer handoff requires the 0.6.5 activation contract", async () => {
  assert.equal(minimumAttendedHandoffExtensionVersion("smartrecruiters"), "0.6.5");
  assert.equal(minimumAttendedHandoffExtensionVersion("jobvite"), "0.6.5");
  assert.equal(minimumAttendedHandoffExtensionVersion("icims"), "0.6.5");
  assert.equal(minimumAttendedHandoffExtensionVersion("bamboohr"), "0.6.5");
  assert.equal(minimumAttendedHandoffExtensionVersion("oraclecloud"), "0.6.5");
  reset(() => ({ ok: true, signedIn: true, version: "0.6.4" }));
