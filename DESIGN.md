...
- **Say each point once, in its strongest spot.** Submit-ownership lives in
  the refusal list ("Never auto-sent"), nowhere else; it left the hero on
  2026-07-28 with the subhead rewrite, so this rule no longer names the hero.
  Data handling lives in the same trio ("Never sold") and on `/privacy`, not
  in the hero either. The free-tier claim lives in the pricing headline. The
  cancel-easy line lives on the pricing card and in Settings. Repetition
  reads as insecurity.
...
|------|----------|-----------|
| 2026-08-14 | The verified Stripe return is a Litos receipt printer: one blue payment rail moves through processing, stepped line-by-line printing and completion before revealing one paper receipt with the authenticated paid offer amount, cadence, time and reference. The supplied printer component defines the interaction rhythm; Litos tokens and native CSS carry it without adding a second UI framework. Reduced motion resolves directly to the static completed receipt. | Extends the existing receipt motif from packet speed to payment confirmation without adding a new visual language. The amount comes from the backend ledger, never the URL or client memory, so the memorable interaction cannot misstate what Stripe charged. |
...
| 2026-07-04 | Receipt motif (mono timestamp log) as the hero artifact | Speed shown as fact, not hype; it serves "calm + speed" without violating the no-urgency law |
...
| 2026-07-04 | Submit-ownership language softened from absolutes ("You hit submit", "submit stays yours", "never submitted for you") to "You get the final say" / "You review before anything is submitted" | Opt-in auto-submit exists with a cancelable countdown (**15 seconds**, corrected 2026-07-27, see the row below), so "always yours" was technically false; privacy policy states the countdown explicitly |
...
| 2026-07-23 | **The Stack replaces the Dart as the mark.** One artwork everywhere: a black stack of four tapering bars over a solid block, centred and sheared 6 degrees, on a plain white ground. No reversed, tinted or accent-coloured variant exists, by design. All of `public/brand/` and `app/icon.png` are now generated by `scripts/generate-brand-assets.mjs`; edit MARK_PATH there and re-run rather than hand-editing any asset. `opengraph-image.tsx` inlines the same path. Two wrappers that assumed the old mark were corrected: the `/start` founder note no longer rounds or pads it (the artwork is a square with its own ground and the corners were being clipped) and the `/try` popup mock no longer sits it on a `bg-brand` tile. Unreferenced `litos-logo-blue.svg` and `litos-logo-white.svg` deleted as old branding | The mark carries its own white ground, so it never needs a coloured tile, a circle, or padding; and one generator means the favicon, the OG card, the header and the extension can never drift apart |
...
| 2026-07-23 | Hero opening replaced: the resume-storm sting + locked transition videos gave way to the Application Roll, a live WebGL drum (three r128, `components/cinema/paperRollEngine.js`) wandering an endless white floor and printing the job hunt as 24 unique hyperreal sheets (5 resumes, postings, portal forms, an online assessment, interview invites, rejections, offers, the tracker spreadsheet) on a 6x4 grid atlas resolved per-fragment in the shaders, so nothing repeats on screen. Same contract as the sting, plus reversibility: loops until first scroll, dissolves into the film scrub, and fades back in (drum paused while hidden, resumed on return) when the viewer scrolls back to the very top; the cursor draws the drum toward it within a bounded, wander-sized reach (reactive-to-you, never yankable; the standalone demo keeps full steering); reduced motion skips it (the film poster carries); runs on mobile at capped pixel ratio; resumes on the roll stay black and white per the footage rule | Mehek: the hero should showcase the Application Roll. The opening slot was already the page's one pre-scroll looping element, so "things settle, one thing loops" still holds |
| 2026-07-23 | Hero subhead: the opening sentence "Litos is a free Chrome extension for students and new grads." deleted; the subhead now opens directly on the mechanism ("Open a posting and it tailors your resume..."). This supersedes the 2026-07-05 Simplify-audit line that added the mechanism/trigger/audience naming to the subhead | Mehek: "that's not what it is" |
| 2026-07-23 | Calibration card shipped: a bottom-right rq-glass dialog appears once per visitor, only after three sections are behind them (the hero roll plus the receipt and odds chapters; it enters as #formats reaches the viewport, and there is no timer, so hero-watchers are never interrupted) and asks three taps: hunt (Summer 2027 internship / new-grad Class of 2027 / experienced full-time / anything open now), field (6 options), region (US / UK / Middle East / anywhere). The payoff names their market in one headline, shows up to three matched real programs from a curated feed (`lib/rolesFeed.ts`: stable careers-hub links only, honest cycle statuses, hard deadlines only where tracker-verified Jul 2026), cites one real stat (nearly 70% of interviews go to first-week applicants, per Lever recruiting benchmarks), then funnels to /start with the profile in the query string. X respects the visitor: permanent dismiss before answering, collapse to a mono YOUR ROLES pill after; answers live in localStorage only. The degree question was cut as the least differentiating of the four proposed | Mehek: personalize the site per visitor with 2-3 questions max, land on a matched role plus an early-applicant stat, then push into trial onboarding. Guardrails held: no fake freshness, no invented numbers, urgency only as verified deadlines in mono ink |
| 2026-07-23 | Calibration handoff completed + tracker-fed feed: (1) the /start Focus step now seeds from the calibration profile in localStorage (survives the login round-trip, which drops query params), shows a mono "calibrated from your homepage answers · adjust freely" caption, and every chip stays editable; the hunt/field to targeting-vocab mapping is unit-tested against lib/periods.ts. (2) `lib/rolesFeed.ts` gains a generated section written by `scripts/sync-roles-feed.mjs` from the internship tracker's data.js: 10 open roles with the tracker's own verified dates and real deadlines (McKinsey London closes Aug 11, Equinor Aug 27, Ares Aug 31, Perella Weinberg Oct 1, D.E. Shaw Dec 31), deduped against the curated list and capped; rerun the script whenever the tracker updates, then test, build, deploy | Mehek approved both flagged next steps; feed freshness now comes from her verified tracker rather than hand-curation alone |
| 2026-07-24 | Calibration card clarity pass (Mehek: too crowded on mobile, background bleeding through): the card is now near-opaque (0.96 white; the default 55% rq-glass let dense mockup sections collide with the payoff text), the match card's mono meta splits into a status+regions line and a verified line, secondary matches drop status noise to quiet one-liners, small print is one line, and the third tap triggers a 1.4s ThinkingOrb "searching" beat (size 20 + mono caption, the same idiom the dashboard, /start, /try and PacketDemo already use) before the reveal, skipped under reduced motion. Also fixed: a hard jump past #formats between rendering frames (native anchor, restored scroll) never intersects the observer, so a passive position check now backs it up | One loading language everywhere the product thinks; opacity beats frost when the backdrop is dense |
| 2026-07-27 | **Ten prior rulings reopened and overridden by Mehek**, after the 10-product competitor flow audit (vault: `competitor-flow-audit-2026-07-27/`, reconciled in `00-DELTA-vs-LITOS.md`). APPROVED, each superseding the ruling named: (1) **onboarding progress indicator** on `/start`, superseding the 07-04 removal of the step-count bar; (2) **explicit skip control** on `/start`, superseding "Finish later is the same code path"; (3) **product preview beside the `/login` gate**, superseding "`/try` carries this before the gate"; (4) **per-pillar CTAs** on the homepage, superseding "per-section CTA buttons NOT adopted, one ask per page" (07-05); (5) **the consolidation phrase may repeat**, superseding say-once for this phrase only; (6) **static product screenshot in the hero** alongside the Application Roll, superseding 07-23; (7) **a conventional promo video** separate from the scroll film, for surfaces the film cannot reach (store listing, social); (8) **a named sub-brand for the agent** ... **REVERSED 2026-07-27, see the row below: the agent stays unnamed**; (9) **harder human-gate marketing**, constrained below; (10) **an institution/outcome proof row**, refused as specified and substituted, see below. DECLINED and still in force: multiplier/outcome claims in copy (S9), a third pricing tier (S41), additional auth providers beyond Google + email code (S46), emoji dividers in the store listing (S20) | Mehek's calls, item by item, against the audit's cross-cut. The Guardrails themselves (no streaks/badges/daily-login rewards, real scarcity, explicit send buttons, cancel parity, colour never encodes urgency) are UNCHANGED and remain non-relitigable |
| 2026-07-27 | **Two constraints attached to the approvals above, neither of them optional.** (a) **Human-gate copy may not restate the absolute.** The 07-04 softening stands on its facts: opt-in auto-submit ships with a cancelable countdown (15 seconds), so "you always hit submit" / "never submitted for you" remain false. Stronger marketing of the review step must be true with that feature switched on, and the countdown stays stated in the privacy policy. (b) **The institution proof row was REFUSED as approved and substituted.** The only relationship to UW, Cal Poly SLO, WVU, Penn and MSU is that their *public sample resume PDFs* were used as test fixtures for the onboarding runs. Naming or badging them as recommenders would fabricate an endorsement and misrepresent five real institutions. Shipped instead: the true, citable rigour claim "tested on 45 real resumes from five university career centres" (vault: `litos-onboarding-15-resume-run-2026-07-27.md`), with no logos and no implied endorsement. If a real USC or career-services relationship is ever signed, the endorsement row becomes available on its own evidence | Zero fake numbers was never only about numbers; borrowed authority is the same failure with a logo instead of a digit. The audit found every one of the ten competitors publishing claims their own surfaces contradict, and the whole point of this pass was not to join them |
| 2026-07-27 | **The auto-submit countdown is 15 seconds, not 9. This ledger was wrong and `/privacy` was right.** Source of truth is `COUNTDOWN_SECONDS = 15` in the extension's `src/entrypoints/content.ts`, cross-checked against the 15000ms in `src/lib/overload.ts`. The 2026-07-04 submit-ownership row said "9-second cancelable countdown" and has been corrected in place. **The other two 9-seconds in this log are a different number and are correct:** the receipt (`19:42:07 POSTING DETECTED → 19:42:16 PACKET READY`) is how long the packet takes to build, and stays. The error was borrowing the packet's figure for the countdown. Homepage copy now states the 15 seconds outright rather than saying "a countdown", which was the hedge written while the two surfaces disagreed | Found while writing the human-gate copy in #46, which deliberately shipped without a number because marketing must not pick a side in a contradiction between two of our own surfaces. Now that the source settles it, the specific number is the stronger claim: Jobscan's most-praised trait in its whole review corpus is publishing its own thresholds honestly, and a hedge reads as one when the real figure is available |
| 2026-07-27 | **The agent stays unnamed. Override 8 of 10 was approved in principle, then reversed once the naming options were on the table, and the original ruling stands.** Two findings decided it. First, "Copilot" is the most crowded word in the category: Simplify ships *Simplify Copilot*, Careerflow ships *Career Copilot*, and Jobright leads with *"Your AI Job Search Copilot"*, so the obvious name would have echoed the market leader in the one place we were trying to sound distinct. Second, and the sharper problem: in all three of those products the named agent is a CONVERSATIONAL surface you ask questions (Jobright's Orion, AIApply's Interview Buddy). The Litos agent tailors a resume, fills a form, drafts an email and stops. Naming it Copilot would have promised a chat the product does not ship, which is exactly the kind of small mismatch the plain-language bar exists to catch | Mehek's call. A named agent is a retention device for chat products; Litos's agent finishes and gets out of the way, and that is a different thing worth not muddying. The naming budget goes elsewhere |
| 2026-07-28 | **Hero subhead replaced and the hero privacy caption removed.** Subhead is now "Nothing is reused. Every job gets its own resume, form, and email.", superseding "Open a job. We fix your resume, fill in the form, and write the email. You hit send." (07-23). Two reasons. The **trigger is no longer true**: `/dashboard/jobs` is "Every job Litos has found for you" and `/dashboard/applications` runs to "Sent to the company.", so the product no longer requires the user to be standing on a posting. And the hero card was carrying **49 words of body text in two blocks of near-equal length**, so neither read as the point; it is 12 now. The line deliberately does NOT claim submitting from Litos works everywhere, because that is the Greenhouse / Lever / Ashby `detectPortal` allowlist. **The privacy caption is gone from the hero**, superseding item S26 of the 07-05 Simplify-audit pass (permissions + no-data-sale caption at the decision point). Two thirds of it was already said verbatim in the `#faq` refusal trio and the FAQ, against say-once; the deletion clause, the only part said nowhere else, moved into the "Never sold" card, where it reads "you can delete everything whenever you want". **Known cost, so nobody rediscovers it as a bug:** there is no longer proof under the hero CTA. `/try` and the footer still link `/privacy`. PR #67 | Mehek's calls, 2026-07-28. The hero states the differentiator (nothing is reused) rather than the category's most crowded claim; trust copy consolidates where the other refusals already live |
| 2026-07-28 | **The founder line is removed from `#close`.** "Made by Mehek, a student at USC. She is looking for a job too." is gone; the section ends on the receipt line. **Consequence, recorded because it is a decision and not an oversight:** this was the only place the site said a person is behind Litos. The footer keeps an X link and no human voice. The 2026-07-27 flow audit found a human voice inside the flow in only 1 of 10 competitors, and the social-proof gate lists this line among what shipped in place of user counts, which stay off the site until 50 users. PR #68 | Mehek's call, 2026-07-28 |
