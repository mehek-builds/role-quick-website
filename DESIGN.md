...
| 2026-07-07 | The Dart logo cut into production (approved 2026-07-05, was pending). `public/brand/rolequick-mark.svg` replaced with the paper-dart tile (two blue triangles on a `#eef1fe` tile), so the header, footer, and login mark all update from one file; favicon (`app/icon.png`) and the mark/tile PNGs (16→512) regenerated from the same SVG; the OG card (`opengraph-image.tsx`) now inlines the Dart as a data-URI instead of the CSS "R" circle. Wordmark kept as single-ink "RoleQuick" text (the decision's lowercase two-tone treatment left for a later brand pass). The dart doubles as brand rhyme: the mark is itself a flying paper, echoing the storm | One drawing of the mark everywhere the brand appears; the paper-dart ties the logo to the film's flying-paper language |
