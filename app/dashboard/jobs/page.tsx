      ...
      {error && jobs === null ? (
        ...
        <>
          ...
          <ul className="grid grid-cols-1 gap-3">
            {jobs.map((job) => (
              <li key={job.id}>
                <JobRow job={job} application={jobApplicationFor(job, applications)} applied={isJobApplied(job, applications)} match={badgeMatchFor(job, matches[job.id])} preferredLocations={targeting?.locations ?? []} />
              </li>
            ))}
          </ul>
